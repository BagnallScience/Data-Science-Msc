...
# Poisson model explanation
# The Poisson model is suitable for count data like shutouts because it assumes non-negative integer values and models events occurring independently over a fixed interval.
```
...
# Comments:
# - Observe trends in shutouts as a function of innings.
# - Left-handed vs. right-handed pitchers may have different distributions.
# - Jitter helps readability by reducing overlap.
```
...
# Interpretation:
# - Coefficients represent log-multiplicative effects on shutouts.
# - P-values test the null hypothesis that a coefficient is zero.
```
...
# Assess the importance of teamID
# Check the variance of the random effect associated with teamID.
```
...
# Comments:
# - Look for patterns in residual variance (e.g., heteroscedasticity).
# - Unusual patterns may indicate model misfit or non-linearity.
```
...
# Explanation:
# - The coefficients for height/weight reflect their effect on shutouts, all else being equal.
# - Positive coefficients imply that taller/heavier pitchers pitch more shutouts.
```
